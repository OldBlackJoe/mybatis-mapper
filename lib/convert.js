  ...
  try{
    ...
    for (var i=0; i<keys.length; i++){
      ...
      if (isDict(param[keys[i]])) {
        replaceParam = ' param.' + keys[i] + '.';
      } else {      
        replaceParam = ' param.' + keys[i] + ' ';
      }
      
      var paramRegex = new RegExp('(^|[^a-zA-Z0-9])(' + keys[i] + ')($|[^a-zA-Z0-9])', 'g');
      var matches = evalString.match(paramRegex);
      
      if (matches != null && matches.length > 0) {
        for (var z = 0; z < matches.length; z++) {
          var regex = new RegExp('(^|[^a-zA-Z0-9])(' + keys[i] + ')($|[^a-zA-Z0-9])', 'g');
          var m = regex.exec(matches[z]);

          evalString = evalString.replace(m[2], replaceParam);
        }
      }
    }
