    ...
    for (var z = 0; z < matches.length; z++) {
      ...
      if (collection != null && collection.length > 0) {
        ...
        for (var co = 0; co < collection.length; co++) {
          if (isDict(collection[co])) {
            var keys = Object.keys(collection[co]);            
            var txt = baseText;
            
            for (var i=0; i<keys.length; i++){             
              var strRegex = new RegExp("\\#{" + item + "\\." + keys[i] + "}", "g");
              var intRegex = new RegExp("\\${" + item + "\\." + keys[i] + "}", "g");
              
              txt = txt.replace(strRegex, "\"" + collection[co][keys[i]] + "\"");
              txt = txt.replace(intRegex, collection[co][keys[i]]);
            }
            
            // Convert <if> in <foreach>
            var tempParam = {};
            tempParam[item] = collection[co];            
            txt = convertIf(txt, tempParam);
            txt = txt.replace(/^\s*$/g,"");
            ...
            txt = txt.replace(intRegex, collection[co]);
            
            txts.push(txt);
          }
        }

        forString += txts.join(separator);
      }
